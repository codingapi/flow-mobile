...
import {FlowSubmitResultParser} from "../domain";
...
export class FlowEventContext {
    ...
    startFlow = (callback?: (res: any) => void) => {
        ...
        FlowApiContent.getInstance().startFlow(body)
            .then(res => {
                if (res && res.success) {
    ...
    submitFlow = (approvalState: boolean, callback?: (res: any) => void, operatorIds?: number[]) => {
        this.validateForm().then((validateState) => {
            if (validateState) {
                ...
                this.flowStateContext.setRequestLoading(true);
                FlowApiContent.getInstance().submitFlow(body)
                    .then(res => {
                        if (res && res.success) {
                            if (callback) {
                                callback(res);
    ...
    removeFlow = (callback?: (res: any) => void) => {
        ...
        FlowApiContent.getInstance().removeFlow(body).then(res => {
            if (res && res.success) {
                if (callback) {
                    callback(res);
                }
    ...
    saveFlow = (callback?: (res: any) => void) => {
        this.flowStateContext.setRequestLoading(true);
        const body = this.getRequestBody();
        FlowApiContent.getInstance().saveFlow(body).then(res => {
            if (res && res.success) {
                if (callback) {
                    callback(res);
                }
    ...
    customFlow(buttonId: string, callback?: (res: any) => void) {
        this.validateForm().then((validateState) => {
            ...
            if (validateState) {
                ...
                this.flowStateContext.setRequestLoading(true);
                FlowApiContent.getInstance().triggerCustomFlow(body)
                    .then(res => {
                        if (res && res.success) {
                            if (callback) {
                                callback(res);
    ...
    transferFlow(user: FlowUser, callback?: (res: any) => void) {
        this.validateForm().then((validateState) => {
            if (validateState) {
                ...
                this.flowStateContext.setRequestLoading(true);
                FlowApiContent.getInstance().transferFlow(body)
                    .then(res => {
                        if (res && res.success) {
                            if (callback) {
                                callback(res);
    ...
    urgeFlow(callback?: (res: any) => void) {
        ...
        FlowApiContent.getInstance().urgeFlow(body).then(res => {
            if (res && res.success) {
                if (callback) {
                    callback(res);
                }
    ...
    recallFlow(callback?: (res: any) => void) {
        ...
        FlowApiContent.getInstance().recallFlow(body).then(res => {
            if (res && res.success) {
                if (callback) {
                    callback(res);
                }
    ...
    trySubmitFlow(callback?: (res: any) => void) {
        this.validateForm().then((validateState) => {
            if (validateState) {
                ...
                this.flowStateContext.setRequestLoading(true);
                FlowApiContent.getInstance().trySubmitFlow(body)
                    .then(res => {
                        if (res && res.success) {
                            if (callback) {
                                callback(res);
